...
export default function VerbfyExamPage() {
  ...
  const category = useMemo(() => 'Exam', []);

  useEffect(() => { freeMaterialsAPI.getLevels().then((r) => { if (Array.isArray(r?.data)) setLevels(r.data); }); }, []);
  ...
  return (
    <div className="min-h-screen bg-white">
      <Head><title>VerbfyExam</title></Head>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-900">VerbfyExam</h1>
          <p className="text-gray-600 mt-2">{locale === 'tr' ? 'Deneme ve sınavlarla hedef puanına hazırlan.' : 'Prepare for your target score with mock exams and tests.'}</p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(materials || []).map((m) => { const id = (m as any)._id || (m as any).id || m.title; const isSel = !!selected[id]; return (
              <div key={id} className="group bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                <div className="relative h-40"><Image src={m.thumbnailUrl || '/images/verbfy-exam.png'} alt={m.title} fill className="object-contain p-4 group-hover:scale-105 transition-transform duration-200" /></div>
                <div className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between text-sm text-gray-500">
                    <span>{m.level || '-'}</span>
                    <button onClick={() => toggle(m)} className={`px-3 py-1 rounded-md border ${isSel ? 'bg-red-600 text-white border-red-600' : 'border-gray-300 hover:bg-gray-50'}`}>{isSel ? (locale === 'tr' ? 'Seçildi' : 'Selected') : (locale === 'tr' ? 'Seç' : 'Select')}</button>
                  </div>
                </div>
              </div>
            ); })}
